...
class AmountsMixin:
    ...
    _FNS_GRAND_VSEGO_RE = re.compile(
        ...
        r"(\d[\d\s]*[.,]\d{2}|\d[\d\s]*\d)\s*(?:руб\w*|рублей|py6\w*|р\.)",
        re.IGNORECASE,
    )
    ...
    # «2 равных поля»). 2-я очередь у ФНС — недоимка (зарплатные/удержанные платежи),
    # 1-я и 3-я — основной долг/налог. Слот заполняется значением Total.
    _FNS_QUEUE_DEFAULT_SUF = {1: "LoanDebt", 2: "Arrears", 3: "LoanDebt"}
    ...
    def _apply_fns_queue_finances(self, fields: Dict[str, Any], text: str) -> None:
        ...
        #   • есть только Total → вся сумма очереди = одна категория, равная Total
        #     (слот по номеру очереди: 2-я → недоимка, иначе → осн.долг/налог) →
        #     получаем 2 РАВНЫХ поля;
        ...
        for n, data in list(queues.items()):
            ...
            if total and not cats:
                data[self._FNS_QUEUE_DEFAULT_SUF.get(n, "LoanDebt")] = total

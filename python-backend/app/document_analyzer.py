...
class DocumentAnalyzer(ClassifyMixin, PartiesMixin, AmountsMixin, IpExtractionMixin, ObligationsMixin, InflectionMixin, PriorCollectionMixin):
    ...
    #
    # Список реквизитов ниже (ИНН/ОГРН/…) был написан руками и закрывал ровно
    # пять меток из реестра; всё остальное проходило насквозь: «…, кв 70 Дата
    # рождения: 30.12.1984», «…кв.29 Финансовый управляющий: ОВЧ…». Здесь тот же
    # приём, но метки берутся из РЕЕСТРА — новый банк со своим написанием
    # закрывается правкой данных, а не этой функции.
    #
    # Разделитель (двоеточие или тире) ОБЯЗАТЕЛЕН: без него правило режет адрес
    # по слову, которое просто похоже на метку. Отсюда же `[\s,;]+` — обрезаем
    # только ХВОСТ, метка в начале значения это другой случай (там нечего
    # оставлять слева).
    #
    # Замер на 101 документе: срабатывает на 3 свежих заявлениях, на корпусе из
    ...
    def _sanitize_address_fields(self, fields: Dict[str, Any]) -> None:
        ...
        обрезаем хвост по первому такому маркеру; затем — по подписи любого
        следующего поля из реестра меток; если осмысленного адреса (букв) не
        осталось — поле было мусором, удаляем. Затем режем склейку двух
        ...
        for k in ("address", "applicantAddress", "creditorAddress", "managerAddress",
            ...
            cleaned = self._ADDRESS_TAIL_LABEL_RE.split(
                cleaned, maxsplit=1
            )[0].strip().rstrip(" ,;-")
            ...
            if field_name in summable_fields:
                ...
                for i, pattern in enumerate(field_patterns):
                    ...
                    if match:
                        ...
                        cleaned_value = None
                        # Фильтр длины отсеивает текстовый мусор («г.», «в»), но для
                        # ЧИСЛА два символа — нормальное значение: ставка неустойки
    ...
    def _extract_all_collateral_items(self, text: str) -> List[str]:
        ...
        _inventory_re = re.compile(
            ...
            # «Должнику принадлежат на праве собственности: Недвижимое имущество: 1) …»
            # — опись собственности должника, а не залог. Двоеточие обязательно:
            # оно отличает ЗАГОЛОВОК перечня от оборота внутри описания залога
            # («объект, принадлежащий залогодателю на праве собственности»), где
            # предмет настоящий и терять его нельзя.
            r"|принадлеж\w+[^.:]{0,60}на\s+прав[еи]\s+собственност\w*\s*:",
            re.IGNORECASE,
        )

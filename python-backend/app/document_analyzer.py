...
class DocumentAnalyzer(ClassifyMixin, PartiesMixin, AmountsMixin, IpExtractionMixin, ObligationsMixin, InflectionMixin):
    ...
    # СРО по формуле членства в просительной части заявлений-инициирований ЮЛ:
    # «утвердить … управляющего из числа членов СОЮЗА … «АВАНГАРД» (ИНН …)»,
    # «члена Ассоциации «… «Меркурий» (ОГРН …)», «из числа членов НП "ЦФОП АПК" - …».
    # Берём от правовой формы СРО до открывающей скобки реквизитов/конца строки.
    _SRO_MEMBERSHIP_RE = re.compile(
        # [\s.]+ терпит склейку «членов.НП» (pdf2docx съел пробел).
        r"(?:из\s+числа\s+членов|член(?:а|ом|ов)?)[\s.]+"
        r"((?:СОЮЗ|Ассоциаци|Некоммерческ|\bНП\b|Саморегулируем|ААУ)[^\n(]{3,220}?)"
        r"\s*(?:\(|,\s*(?:ИНН|ОГРН)|\n|$)",
        re.IGNORECASE,
    )

    def _resolve_manager_sro(self, fields: Dict[str, Any], text: str) -> None:
        """Извлекает и/или нормализует СРО управляющего.

        1) Если `sroName` пуст — вытаскиваем название из формулы членства
           («из числа членов <СРО>» / «члена <СРО>») в заявлениях-инициированиях ЮЛ,
           где основной парсер молчит (нет «СРО »/«[987]»).
        2) Сопоставляем с реестром (`sro_data`) и заменяем каноничным NAIM_FULL;
           если в реестре нет (Авангард/Меркурий) — оставляем очищенное сырое имя."""
        from sro_registry import resolve_sro

        membership_set = False
        if not (fields.get("sroName") or "").strip() and text:
            m = self._SRO_MEMBERSHIP_RE.search(text)
            if m:
                raw = re.sub(r"\s+", " ", m.group(1)).strip(" ,.;-")
                # Хвостовые реквизиты («… «Созидание» ОГРН: 102…», «…«ДЕЛО», адрес:
                # 125284…») — обрезаем.
                raw = re.split(r"\s*,?\s+(?:ОГРН|ИНН|КПП|адрес)\b", raw)[0].strip(" ,.;-")
                # Дубль имени через дефис («… «Эгида» -Ассоциация … «Эгида»») —
                # оставляем до первого ЗАКРЫТОГО кавычками имени.
                mdup = re.match(r'(.*?[«"][^»"\n]{2,}[»"])\s*[-–]\s*(?=\w)', raw)
                if mdup:
                    raw = mdup.group(1)
                # Качество: принимаем только ЗАКРЫТОЕ кавычками имя СРО
                # («…«АВАНГАРД»») ИЛИ то, что опознаётся реестром. Иначе это
                # обрывок/мусор («саморегулируемой организации ,САУ») — не берём.
                has_quoted = re.search(r"[«\"][^»\"\n]{3,}[»\"]", raw)
                if len(raw) >= 5 and (has_quoted or resolve_sro(raw)):
                    fields["sroName"] = raw
                    membership_set = True

        cur = fields.get("sroName")
        full = resolve_sro(cur)
        if full:
            # Гард ТОЛЬКО для membership-значений: resolve_sro на straight-кавычках
            # («Ассоциация "Саморегулируемая … "Меркурий"») мог зацепиться за общую
            # фразу и вернуть чужую запись (Меркурий→Эгида). Применяем реестровое имя
            # лишь если оно делит отличительное (не-общее) слово с исходником; иначе
            # оставляем очищенное сырое. Значения из основного парсера не трогаем.
            if not membership_set or self._sro_distinctive_match(cur, full):
                fields["sroName"] = full

    # Общие слова названий СРО — не отличительные (есть почти в каждом).
    _SRO_GENERIC_WORDS = frozenset({
        "саморегулируемая", "саморегулируемой", "организация", "организации",
        "арбитражных", "управляющих", "ассоциация", "ассоциации", "союз", "союза",
        "некоммерческое", "партнерство", "партнерства", "межрегиональная",
        "межрегиональной", "профессиональных", "сро", "объединение",
    })

    @classmethod
    def _sro_distinctive_match(cls, raw: Optional[str], full: str) -> bool:
        """У `raw` и `full` есть общее ОТЛИЧИТЕЛЬНОЕ слово (напр. «меркурий»,
        «авангард»). Если у `raw` вообще нет отличительных слов — не мешаем (True)."""
        def dist(s: str):
            return {w for w in re.findall(r"[а-яёa-z]{4,}", (s or "").lower())
                    if w not in cls._SRO_GENERIC_WORDS}
        rd = dist(raw)
        if not rd:
            return True
        return bool(rd & dist(full))

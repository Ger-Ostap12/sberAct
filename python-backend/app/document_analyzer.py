...
class DocumentAnalyzer:
        ...
        patterns = {
            "rtk_application": [
                # Паттерны для заявления о включении в РТК
                {
                    "name": "applicantName",
                    "patterns": [
                        r"\[2\]\s*((?:ООО|ОАО|ПАО|ЗАО|АО|Общество\s+с\s+ограниченной\s+ответственностью)[^,\n\[]+)",  # Приоритет: [2] перед названием организации
                        ...
                        r"(?:должник|заявитель)[:\s]+((?:ООО|ОАО|ПАО|ЗАО|АО|Общество\s+с\s+ограниченной\s+ответственностью)[^,\n\[]+)",  # Организация после "Должник:"
                        ...
                        r"([А-ЯЁ]{2,}\s+[А-ЯЁ]{2,}\s+[А-ЯЁ]{2,})"  # Общий паттерн ФИО
                    ],
                    "type": "person_name"
                },
                {
                ...
                },
                {
                    ...
                    "patterns": [
                        ...
                        r"ИНН[:\s]*([0-9]{10,12})",  # Общий шаблон ИНН
                    ],
                    "type": "inn"
                },
                {
                    "name": "ogrn",
                    "patterns": [
                        r"Должник[:\s][\s\S]{1,200}?ОГРН[:\s]*([0-9]{10,15})",  # приоритет: ОГРН в блоке должника
                        r"должник[:\s][\s\S]{1,200}?ОГРН[:\s]*([0-9]{10,15})",
                        r"ООО\s*«?[^\n]+»?\s*,?\s*ОГРН[:\s]*([0-9]{10,15})",
                        r"ОГРН[:\s]*([0-9]{10,15})",
                        r"ОГРН\s*№?\s*([0-9]{10,15})"
                    ],
                    "type": "ogrn"
                },
                {
                ...
                },
            {
                ...
                },
                {
                    "name": "creditorName",
                    "patterns": [
                        r"кредитор[:\s]+([^,\n]+)",  # Кредитор: ПАО Сбербанк (ПРИОРИТЕТ!)
                ...
                },
                {
                    ...
                    "patterns": [
                        ...
                        r"должник[:\s]+((?:ООО|ОАО|ПАО|ЗАО|АО|Общество\s+с\s+ограниченной\s+ответственностью)[^,\n\[]+)",  # Организация после "должник:"
    ...
    def analyze(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if document_type in ["ip_enforcement_statement", "ip_enforcement_statement_collateral",
    ...
    def _normalize_mortgage_interface_fields(self, fields: Dict[str, Any], text: str):
        ...
        fields["applicantName"] = _strip_procedure_phrase(self._strip_ooo_prefix(fields.get("applicantName")))
    ...
    def extract_fields(self, text: str, document_type: str) -> Dict[str, Any]:
        ...
        if debtor_block_match:
            ...
            # Извлекаем ОГРН
            ogrn_match = re.search(r"ОГРН[:\s]*([0-9\s]{9,15})", debtor_block, re.IGNORECASE)
            if not ogrn_match:
                ogrn_match = re.search(r"([0-9\s]{9,15})\s*\[3\]", debtor_block)
            if ogrn_match:
            ...
            # Извлекаем ИНН
            inn_match = re.search(r"ИНН[:\s]*([0-9\s]{9,15})", debtor_block, re.IGNORECASE)
            if not inn_match:
                inn_match = re.search(r"([0-9\s]{9,15})\s*\[4\]", debtor_block)
            if inn_match:
        ...
        if source_doc_type.startswith("initiation_legal"):
            ...
            if debtor_block:
                ...
                if company_match:
                    ...
                    # По требованию: в полях должно быть только название без "ООО/Общество с ограниченной ответственностью".
                    # Поэтому:
                    # - debtorName / applicantName / legalShortName = короткое название (например, "Азбука")
                    # - полное наименование с ОПФ при необходимости всегда можно восстановить по шаблону в шаблоне акта.
                    name_core = short_name or full_name
                    extracted_fields["debtorName"] = name_core
                    extracted_fields["applicantName"] = name_core
        ...
        if detected_entity_type:
            ...
            if detected_entity_type == "legal":
                # Для юрлиц используем короткое название без приставок "ООО / Общество с ограниченной ответственностью"
                ...
                if name_source_legal:
                    ...
                    if short_legal_name:
                        ...
                        extracted_fields["applicantName"] = short_legal_name
                ...
                inn_clean = extracted_fields.get("companyInn") or extracted_fields.get("inn")
                if inn_clean:
    ...
    def extract_ip_enforcement_fields(self, text: str) -> Dict[str, str]:
        ...
        set_field(
            ...
            [
                r"должник[:\s]*\n([А-ЯЁ][а-яё\s]{10,50}?)(?:\n|$|[,\[])",
                r"ответчик[:\s]*\n([А-ЯЁ][а-яё\s]{10,50}?)(?:\n|$|[,\[])",
                r"должник[:\s]+ИП\s+([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){1,2})",
                r"ответчик[:\s]+ИП\s+([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){1,2})",
                r"ИП\s+([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){1,2})",
                r"индивидуальн[а-яё\s]+предпринимател[ья]\s+([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){1,2})",
                r"\[2\]\s*([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){1,2})"
            ],
            flags=re.MULTILINE
        )
        ...
        set_field("inn", [r"ИНН[:\s]+([0-9]{10,12})"])
        set_field("ogrnip", [r"ОГРН[:\s]+([0-9]{13,15})"])
        set_field(
            "snils",
            [
                r"\[5\]\s*([0-9-]{11,14})",
        ...
        set_field(
            ...
            postprocess=lambda value: value.title()
        )

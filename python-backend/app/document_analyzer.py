...
class DocumentAnalyzer:
    ...
    def _extract_ip_financials(self, text, fields, normalized_text):
        """Финансовые поля ИП: показатели кредита, суммы задолженности (основной долг/проценты), неустойки (построчное суммирование) и госпошлина [16]. Вынесено из extract_ip_enforcement_fields."""
    ...
    def _extract_ip_collateral(self, text, fields, normalized_text):
        """Данные о залоге для ИП: номер/дата договора залога, суммы залоговых обязательств, предмет залога [1221], нормализация суда. Вынесено из extract_ip_enforcement_fields."""

...
Извлечение собрано из паттернов, каждое поле извлекается независимо и не
...
Исключение — реквизиты (ИНН/ОГРН/ОГРНИП): их только ПОМЕЧАЕМ. Провал контрольной суммы на OCR-скане обычно значит «свой
реквизит с побитой цифрой», а не «чужой».
...
def check_entries(
    ...
    и битый ИНН доезжает до карточки должника молча. 
    """
    issues: List[Issue] = []

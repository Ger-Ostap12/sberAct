...
# Метка блока третьих лиц. Написания — из реестра меток
# (label_synonyms.THIRD_PARTY_HEADER_LABELS), но с морфологическим зазором:
# буквальная альтернатива из реестра не покрыла бы «Третьи лица» и «Третье лицо»
# одной записью. Сторож соответствия реестру — в tests/test_third_parties.py.
#
# Якорь на НАЧАЛО СТРОКИ обязателен: метка блока стоит в шапке отдельной строкой
# (замер: все 7 вхождений в корпусе), а без якоря блок открывала бы проза
# «…привлечь в качестве третьего лица: …» из тела заявления.
_TP_LABEL_RE = re.compile(
    r"^[ \t]*(?:Треть[ияе]\w*|Заинтересованн\w+)\s+лиц\w*\s*:",
    re.IGNORECASE | re.MULTILINE,
)

# Конец блока третьих лиц. Кроме меток соседних блоков шапки сюда входят
# маркеры НАЧАЛА ТЕЛА заявления: без них блок дочитывал тело до первого
# управляющего, и должник из фразы «…Иванов И.И. признан банкротом» становился
# вторым третьим лицом (свежие-33/34).
_TP_BLOCK_END_RE = re.compile(
    r"\n\s*(?:Финансов\w+\s+управляющ|Временн\w+\s+управляющ|Конкурсн\w+\s+управляющ|"
    r"(?:По\s+)?Дел[оу]\s*№|ПРОШУ|ПРОСИТ|"
    ...
    r"Публичное\s+акционерное\s+общество\s+[\"«]|Цена\s+иска|"
    r"Госпошлин|Государственн\w+\s+пошлин|При\s+определени|"
    # Заголовок и первые слова тела: «Требование о включении в реестр»,
    # «В производстве Арбитражного суда…», «Решением/Определением суда…».
    r"Требовани\w*\s+о\s+включени|В\s+производстве|(?:Решением|Определением)\s+Арбитражн|"
    r"ИСКОВОЕ|ЗАЯВЛЕНИЕ|$)",
    re.IGNORECASE,
)
...
def _third_parties_block(text: str) -> str:
    """Весь блок третьих лиц — до соседней метки шапки или начала тела."""
    m = _TP_LABEL_RE.search(text)
    if not m:
        return ""
    rest = text[m.end():]
    stop = _TP_BLOCK_END_RE.search(rest)
    return rest[: stop.start()] if stop else rest[:800]


# «Денисов А.В.» — форма записи стороны в нумерованном перечне заинтересованных
# лиц. Полного имени здесь нет, и _looks_like_fio такую строку не пропускает
# (в токене «А.В.» точки), поэтому проверку держим отдельно.
...
def _is_party_start(line: str) -> bool:
    ...
    if not s:
        return False
    return bool(_ORG_PREFIX_RE.match(s)) or _looks_like_fio(s) or bool(_FIO_INITIALS_RE.match(s))


# Реквизиты, напечатанные в скобках сразу после наименования: «ООО МФК
# «Быстроденьги» (Почтовый адрес: …, ОГРН: …, ИНН: …, КПП: …)». В карточку они
# уже разобраны отдельными полями, и в имени это проза.
#
# Условие на СОДЕРЖИМОЕ скобок обязательно: «Банк ВТБ (ПАО)» — часть настоящего
# наименования, и резать его нельзя.
_NAME_REQUISITE_TAIL_RE = re.compile(
    r"\s*\((?=[^)]{0,300}?(?:адрес|ИНН|ОГРН|КПП|СНИЛС|"
    r"дата\s+рождения|место\s+рождения|тел\w*[.:\s]))",
    re.IGNORECASE,
)

# Адрес без метки — почтовый индекс и следом название региона: так свёрстан
# нумерованный перечень заинтересованных лиц (корпус-13), где у стороны есть
# ОГРН/ИНН с метками, а адрес идёт голой строкой следом. Индекс не всегда
# начинает строку — у части записей он приклеен к хвосту ИНН.
#
# Границы по цифрам обязательны: без них шестизначный кусок ОГРН (13 цифр) или
# ИНН (12 цифр) сошёл бы за индекс.
_BARE_INDEX_ADDR_RE = re.compile(r"((?<!\d)\d{6}(?!\d)\s*,?\s+[А-ЯЁа-яё][^\n]{8,200})")


def _clean_party_name(name: str) -> str:
    """Снимает с наименования хвост реквизитов в скобках и служебную пунктуацию."""
    m = _NAME_REQUISITE_TAIL_RE.search(name)
    if m:
        name = name[: m.start()]
...
def _parse_party_record(rec_text: str):
    ...
    if not lines or not _is_party_start(lines[0]):
        return None
    name = _clean_party_name(lines[0])
    if not name:
        return None
    d = {"name": name}
    ...
    if not d.get("address"):
        bare = _BARE_INDEX_ADDR_RE.search(rec_text)
        if bare:
            d["address"] = re.sub(r"\s+", " ", bare.group(1)).strip().rstrip(",;")

    return d


# Порядковый номер стороны в перечне: «1: ООО «Перспектива» … 2: ООО «ГК Каскад»».
# Такой номер — более надёжный разделитель записей, чем начало строки: в корпус-13
# двенадцатая сторона начинается ПОСЕРЕДИНЕ строки с адресом одиннадцатой
# («…, ул. Сосновая, д. 11/9 12: Родина Н.А.»).
#
# Заглядывание вперёд на заглавную букву или кавычку отсекает время («17:00») и
# номера пунктов внутри адреса; запрет цифры/точки слева — дробные номера домов.
_TP_ORDINAL_RE = re.compile(r"(?<![\d.,])(\d{1,2})\s*:\s*(?=[А-ЯЁ«\"])")


def _numbered_party_records(block: str) -> list:
    """Записи перечня, нарезанные по порядковым номерам. Пусто — перечня нет."""
    метки = list(_TP_ORDINAL_RE.finditer(block))
    # Один номер ни о чём не говорит: это может быть «1:» из адреса или времени.
    # Перечень начинается с единицы и идёт подряд — иначе это не перечень.
    if len(метки) < 2 or [int(m.group(1)) for m in метки] != list(range(1, len(метки) + 1)):
        return []
    записи = []
    for k, m in enumerate(метки):
        конец = метки[k + 1].start() if k + 1 < len(метки) else len(block)
        записи.append(block[m.end():конец])
    return записи
...
def extract_third_parties(text: str) -> list:
    ...
    if not block:
        return []
    out = []
    нумерованные = _numbered_party_records(block)
    if нумерованные:
        for rec in нумерованные:
            parsed = _parse_party_record(rec)
            if parsed:
                out.append(parsed)
        return out
    ...
    for seg in _TP_LABEL_RE.split(block):
        lines = seg.split("\n")

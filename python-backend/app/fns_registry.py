...
# Индексы: ('mi', num, region_key) / ('uf', region_key) / ('center',) СПИСОК кандидатов.
# Список, а не одна запись: один номер инспекции обслуживает несколько городов через
# ТОРМ (территориально-обособленные рабочие места) — «№ 13 по Ростовской области»
# существует как база + записи по районам/городам (Новочеркасск, Тарасовский р-н…).
# Нужный адрес выбирается по городу-подсказке из шапки заявления (см. resolve_address).
...
def _index() -> Dict[Tuple, list]:
    ...
    def _add(key: Tuple, rec: dict, is_base: bool) -> None:
        ...
        # Дедуп по адресу (в справочнике встречаются точные повторы). Но если повтор
        # оказался базовой записью — поднимаем флаг is_base у уже добавленной (иначе
        # головной адрес инспекции теряет приоритет фолбэка при пустой подсказке:
        # напр. плоская «№13 по РО» = адрес ТОРМ «в г.Новочеркасске»).

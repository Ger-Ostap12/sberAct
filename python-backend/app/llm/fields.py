...
# Порядок блоков = порядок на экране = порядок вызовов LLM.
#
# Это утверждение проверяемое, и один раз оно уже разошлось с правдой:
# представитель стоял ВТОРЫМ, а на форме он идёт ПОСЛЕ ответчиков
# (DocumentAnalysis: суд -> даты -> ответчики -> кредитор -> представитель).
# Юрист ждал разбора блока, до которого ещё не дошёл, вместо ответчиков —
# единственного блока, где подсказки реально появляются (ИНН 64%).
# При правке формы сверяй порядок здесь с порядком секций там.
#
# Юрист читает форму сверху вниз, поэтому раньше должно приходить то, что он
# увидит первым. `title` идёт в индикатор прогресса, `labels` — в текст подсказки.
BLOCKS: tuple = (
    {
        "key": "court",

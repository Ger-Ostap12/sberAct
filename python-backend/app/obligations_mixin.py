...
class ObligationsMixin:
    if TYPE_CHECKING:
        # Реализованы в document_analyzer.DocumentAnalyzer / amounts_mixin.AmountsMixin;
    ...
    def _parse_obligations_fallback(self, extracted_fields, text, obligations):
        ...
        obligation_patterns = [
            ...
            r'договор\s+поручительства[:\s]*от\s+\[104\]\s*№\s*\[114\]',
            # Общий слой по реестру видов основания (`claim_basis`). Стоит
            # ПОСЛЕДНИМ и ничего не вытесняет: совпадения всех паттернов
            # складываются, дубликаты снимает `_dedupe_obligations`.
            #
            # Он закрывает виды, которых в списке выше нет вовсе: судебный
            # приказ, исполнительный документ, соглашение о кредитовании. На
            # свежих заявлениях (31.08) именно из-за них девять блоков были
            # пустыми — разбор ждал только кредитный договор.
        ...
        for i, pattern in enumerate(obligation_patterns):
            ...
            for j, match in enumerate(matches):
                ...
                if contract_number and self._is_valid_contract_number(contract_number):
                    obligation = {
    ...
    def extract_obligations(self, text: str, extracted_fields: Optional[Dict[str, str]] = None) -> List[Dict[str, Any]]:
        ...
        # Резать можно ТОЛЬКО по заголовку, то есть по слову в начале документа.
        # Дальше по тексту «заявление» — обычная речь («обращения в суд с
        # заявлением о включении…»), и обрезка по ней отсекает весь корпус
        # документа вместе с договорами.
        #
        # Замер по корпусу: настоящий заголовок стоит на 7% длины (медиана),
        # худший законный случай — 22%, выше 30% нет ни одного. Документы,
        # озаглавленные иначе («Требование»), давали 84% — и теряли ВСЕ
        # обязательства. Порог 40% — вдвое от худшего законного.
        if statement_start_pos > 0 and statement_start_pos <= len(text) * _TITLE_MAX_OFFSET:
            text = text[statement_start_pos:]
            logger.info(f"Текст обрезан до позиции {statement_start_pos}, длина нового текста: {len(text)}")
        elif statement_start_pos > 0:
            logger.info(
                f"Обрезка ОТМЕНЕНА: слово-якорь на позиции {statement_start_pos} "
                f"из {len(text)} — это не заголовок, а текст по существу")

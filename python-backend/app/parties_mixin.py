...
class PartiesMixin:
    ...
    _ADDR_STOP_TOKENS = [
        ...
        # «e-mail»/«email» тут были, а русского написания не было — и адрес
        # продолжался строкой «Адрес электронной почты: …».
        # Оба написания нужны: с «адрес» — чтобы обрезать строку целиком (иначе
        # в адрес доезжает слово «Адрес»), без него — для строк, где подписи нет.
        "e-mail", "email", "адрес электронной почты", "адрес эл. почты",
        "электронной почты", "эл. почты", "эл.почты",
        "дата рождения", "место рождения", "д.р", "д/р",
    ...
    # правильность. Почтового индекса здесь НЕТ намеренно: он проверяется
    ...
    def _collect_block_address(self, block_text):
        ...
        2) если метки нет — стартует с почтового индекса (в начале строки ИЛИ
           в её середине), требуя, чтобы за индексом шёл географический признак:
           «499999, 53 руб.» тоже похоже на индекс с продолжением;
        ...
        # Голое «адрес» цепляет и «Адрес электронной почты» — отдельного гарда
        # здесь НЕТ намеренно: хвост такой метки целиком срезается стоп-токеном
        # («адрес электронной почты» в `_ADDR_STOP_TOKENS`), метка отбраковывается
        # как пустая, и работу подхватывает правило почтового индекса. Гард на
        # метке был написан и снят: мутационная проверка показала, что он не
        # меняет ни одного исхода — второе правило для того же случая только
        # выглядело бы работающим.
        label_re = re.compile(
        ...
        for i, line in enumerate(lines):
            for m in label_re.finditer(line):
                ...
                # ⚠️ ДВУХКОЛОНОЧНАЯ ШАПКА ЗДЕСЬ НЕ ЧИНИТСЯ — проверено дважды.
                #
                # Симптом настоящий: в такой шапке столбцы перемешаны, за меткой
                # стоит ФИО, а адрес идёт следом —
                #     «Адрес регистрации: Петросян Генрих Суренович
                #      (ИНН 616305248218) 344049, г. Ростов-на-Дону, ул. Еляна…»
                # Хвост обрезается по «ИНН» до «Петросян Генрих Суренович (», и
                # контракт поля вычищает значение: адрес теряется.
                #
                # Обе попытки починить это ЗДЕСЬ дали регрессию (01.09.2026):
                #   1) отвергнуть метку и искать дальше по блоку — цикл уходит в
                #      мотивировку: адрес суда, кусок прозы про судебную
                #      повестку (5 документов);
                #   2) переякориться на индекс ВНУТРИ строки — берётся адрес
                #      СЛЕДУЮЩЕЙ стороны: у «КЛИМОВ» а/я управляющего вместо
                #      адреса должника (7 документов).
                # Причина одна: в перемешанных столбцах соседство по тексту НЕ
                # означает принадлежность. Чинить надо РАЗБОР ДВУХКОЛОНОЧНОЙ
                # ШАПКИ (восстановить колонки), а не этот цикл.
                if tc:
            ...
            if start_idx != -1:
                break
        if start_idx == -1:
            # Метки адреса нет — стартуем с почтового индекса.
            #
            # Индекс ищется и СРЕДИ строки: часть заявлений печатает запись
            # стороны одной строкой и адрес не подписывает вовсе —
            #     «Должник: Рудакова Александра Сергеевна, 11.04.2002,
            #      ИНН 614107678825, 346889, Ростовская обл, Батайск г, …»
            # Прежнее правило требовало индекс в НАЧАЛЕ строки и такой адрес
            # теряло. Условие «за индексом запятая и содержимое» отделяет индекс
            # от прочих шестизначных чисел строки (номер паспорта, номер
            # договора): у тех продолжения адреса нет.
            #
            # ⚠️ Побеждает индекс, встреченный РАНЬШЕ ПО ТЕКСТУ, а не тот, что
            # удобнее правилу. Обратный порядок (сначала все начала строк, потом
            # середины) отдавал адрес СЛЕДУЮЩЕЙ стороны: у «Вернём» за строкой
            # должника идёт строка представителя «344003, …, а/я 44», и она
            # начинается с индекса, а адрес должника стоит в середине первой.
            # Шесть цифр с запятой — это ещё и ДЕНЬГИ («499999, 53 руб., в том
            ...
            for i, line in enumerate(lines):
                ...
                if re.match(r"^\d{6}[,\s]", line):
                    ...
                    m = re.search(r"(?<!\d)\d{6}(?!\d)\s*,\s*\S", line)
                    if m:
                        cand = line[m.start():]
                if not cand:
                    continue
                # Проверяем то, что ИДЁТ ЗА индексом: сам индекс подтверждением
                # быть не может.
                after = re.sub(r"^\s*\d{6}\s*,?\s*", "", cut_at_stop(cand)[0])
                if self._ADDR_GEO_MARKER_RE.search(after):
    ...
    # Следы кривой вёрстки внутри имени: осиротевшая кавычка вплотную к скобке
    # и роль в скобках. Ни то, ни другое частью имени быть не может, а именно
    # так выглядит «КОЛОР« (заемщик)»» — плоское имя должника ООО «КОЛОР».
    ...
    def _name_key(value: Any) -> str:
        """Имя без ОПФ, кавычек и регистра — для сравнения, а не для показа."""
        v = re.sub(r"[«»\"'()]", " ", str(value or ""))
        v = re.sub(r"\b(?:ООО|ОАО|ПАО|ЗАО|АО|ИП)\b", " ", v, flags=re.IGNORECASE)
        return re.sub(r"\s+", " ", v).strip().casefold()

    def _reconcile_flat_debtor_name(self, fields: Dict[str, Any],
                                    debtors: List[Dict[str, Any]]) -> None:
        """Плоское имя должника обязано согласоваться с его КАРТОЧКОЙ.

        Карточка собирается разбором блока «Должник:/Ответчик:» и на корпусе
        оказалась верной там, где плоское поле несло мусор: слово «ФИНАНСОВЫЙ»
        (первое слово соседней колонки шапки «ФИНАНСОВЫЙ УПРАВЛЯЮЩИЙ:»), кусок
        прозы «имеет неисполненные денежные», имя с обрубленным «ОГЛЫ».

        Замер по корпусу: плоское имя расходится с карточкой на 8 документах
        из 69, и три из них — ЗАКОННЫЕ (ипотека с несколькими должниками, где
        плоское поле склеено через запятую), плюс краткая форма ЮЛ против
        полной. Поэтому правило не «всегда брать карточку», а по отношению
        строк:

            равны                     не трогаем
            плоское ⊂ карточки        ЮЛ: краткая форма, оставляем;
                                      физлицо: имя обрублено, берём карточку
            карточка ⊂ плоского       список должников через запятую —
                                      оставляем, если нет следов кривой вёрстки
            не связаны                мусор, берём карточку
        """
        if not debtors:
            return
        карточка = (debtors[0].get("name") or "").strip()
        плоское = (fields.get("debtorName") or "").strip()
        if not карточка or not плоское:
            return

        к, п = self._name_key(карточка), self._name_key(плоское)
        ...
        if п in к:
            ...
            брать_карточку = bool(self._NAME_MANGLED_RE.search(плоское))
        else:
            брать_карточку = True

        if брать_карточку:
            logger.info(
                f"Плоское имя должника разошлось с карточкой: {плоское!r} -> {карточка!r}")
            fields["debtorName"] = карточка
            # Краткое наименование ЮЛ бралось из ТОГО ЖЕ сырого имени, значит
            # тоже мусорное. У физлица его быть не должно вовсе, у юрлица —
            # пересобираем из карточки, снимая ОПФ.
            if физлицо:
                fields.pop("legalShortName", None)
            else:
                краткое = self._strip_ooo_prefix(карточка) or карточка
                fields["legalShortName"] = краткое

    @staticmethod
    def _drop_legal_short_name_for_person(fields: Dict[str, Any]) -> None:
        """У физлица краткого наименования юрлица не бывает.

        Поле заполнялось из того же сырого имени, что и debtorName, без проверки
        на организацию — и несло то ФИО должника, то кусок прозы («не имеет
        возможности», «владеет недвижимым имуществом»).

        Чистить ТОЛЬКО после определения типа лица: classify_mixin читает
        legalShortName как признак юрлица, и ранняя чистка сменила бы тип.
        """
        if (fields.get("entityType") or "").lower() != "legal":
            fields.pop("legalShortName", None)

    # Реквизиты САМОРЕГУЛИРУЕМОЙ ОРГАНИЗАЦИИ, в которой состоит управляющий.
    # Стоят вплотную к его ФИО и утекали должнику:
    ...
    #     «…член САУ «Авангард» (ИНН 7705479434 ОГРН 1027705031320 рег.номер 005»
    # Якорь — слово «член» и окно в 220 знаков: СРО называют по-разному
    # (Ассоциация, Союз, САУ, НП), а вот членство пишут всегда одинаково.
    # Реквизиты СРО берутся по ДВУМ якорям сразу, и одного мало.
    #
    # Только «член» — мало: в просительной части стоит «из числа членОВ которой
    # будет утвержден временный управляющий Общества … «Перспектива» ОГРН: …,
    # ИНН: …», и следом идут реквизиты САМОГО ДОЛЖНИКА.
    # Только метка СРО — тоже мало: должником может быть ассоциация или союз.
    #
    # Поэтому требуем цепочку «член… → метка СРО → реквизит», и реквизит
    # засчитываем лишь тот, что стоит ПОСЛЕ метки.
    _SRO_MEMBERSHIP_RE = re.compile(r"член\w*\s[\s\S]{0,260}", re.IGNORECASE)
    _SRO_MARKER_RE = re.compile(
        r"саморегулируем\w*|ассоциаци\w*|союз\w*|\bСРО\b|\bСАУ\b|\bНП\b|"
        r"партн[её]рств\w*",
        re.IGNORECASE)
    _REQUISITE_IN_WINDOW_RE = re.compile(
        r"(?:ОГРН[:\s\u2116]*(?P<ogrn>\d{13})|ИНН[:\s\u2116]*(?P<inn>\d{10})(?!\d))",
        re.IGNORECASE)

    @classmethod
    def _sro_requisites(cls, text: str) -> set:
        """ОГРН и ИНН саморегулируемой организации — заведомо НЕ реквизиты должника."""
        out = set()
        for окно in cls._SRO_MEMBERSHIP_RE.finditer(text or ""):
            кусок = окно.group(0)
            метка = cls._SRO_MARKER_RE.search(кусок)
            if not метка:
                continue
            # Реквизиты СРО стоят вплотную к её названию и заканчиваются на
            # границе пункта. Без обрезки окно перепрыгивало в следующий пункт
            # просительной части («…оф.14; 5. взыскать с Общества … ОГРН…»),
            # и реквизиты САМОГО ДОЛЖНИКА объявлялись чужими.
            хвост = кусок[метка.end():метка.end() + 200]
            хвост = re.split(r"[;\n]", хвост, maxsplit=1)[0]
            for m in cls._REQUISITE_IN_WINDOW_RE.finditer(хвост):
                for ключ in ("ogrn", "inn"):
                    if m.group(ключ):
                        out.add(m.group(ключ))
        return out

    def _drop_sro_requisites_from_debtor(self, fields: Dict[str, Any], text: str) -> None:
        """Снимает с должника реквизиты СРО управляющего.

        Чинить последствие («у физлица не бывает ОГРН») здесь мало: до чистки
        ОГРН СРО определял ТИП ЛИЦА — detect_entity_type считает тринадцать цифр
        доказательством юрлица и перебивает ФИО. Четыре свежих заявления из-за
        этого объявляли гражданина организацией.

        Вызывать ДО _finalize_debtor_type.
        """
        чужие = self._sro_requisites(text)
        if not чужие:
            return
        for поле in ("ogrn", "inn", "companyInn", "ogrnip"):
            значение = re.sub(r"\D", "", str(fields.get(поле) or ""))
            if значение and значение in чужие:
                logger.info(f"Реквизит СРО у должника: {поле}={значение} — очищено")
                fields.pop(поле, None)

    @staticmethod
    def _drop_ogrn_for_person(fields: Dict[str, Any], debtors: List[Dict[str, Any]]) -> None:
        """ОГРН бывает только у организации; у ИП это ОГРНИП в своём поле.

        Военная ипотека: ОГРН ФГКУ «Росвоенипотека» (третьего лица) оказывался
        у гражданина-должника и в плоском поле, и в карточке.

        Вызывать ПОСЛЕ определения типа лица — иначе нечего спрашивать.
        """
        if (fields.get("entityType") or "").lower() in ("legal", "kfh"):
            return
        if fields.get("ogrn"):
            logger.info(f"ОГРН {fields['ogrn']} у не-юрлица — очищено")
            fields.pop("ogrn", None)
        for карточка in debtors or []:
            if карточка.get("ogrn"):
                карточка["ogrn"] = ""

        # ИНН гражданина — ДВЕНАДЦАТЬ цифр, у организации десять. Плоское поле
        # брало десятизначный ИНН третьего лица (Управление Росреестра в ДДУ),
        # тогда как карточка должника несла верный двенадцатизначный.
        свой = re.sub(r"\D", "", str((debtors or [{}])[0].get("inn") or ""))
        # Плоское поле у ипотеки СКЛЕИВАЕТ ИНН нескольких должников через
        # запятую («644986285759, 644122681613»). Подстановка ИНН первой
        # карточки затирала бы созаёмщика — сверяем поэлементно.
        части = {re.sub(r"\D", "", ч) for ч in str(fields.get("inn") or "").split(",")}
        плоский = re.sub(r"\D", "", str(fields.get("inn") or ""))
        if len(свой) == 12 and свой not in части:
    ...
    def _fill_creditor_requisites(self, fields: Dict[str, Any], text: str) -> None:
        ...
        if len(creditor_name) > 200:
            return
        # Имя нужно ТОЛЬКО для сверки со справочником банков. Прежний выход по
        # пустому имени рубил заодно и извлечение из ДОКУМЕНТА: у «Петросян Г.С.»
        # имя кредитора на этапе паттернов ещё не извлеклось (появляется позже,
        # другим слоем), и юридический адрес «108811, г. Москва, БЦ Comcity…»,
        # который блочный слой находит верно, в поле не попадал вовсе.
        matched = _match_creditor_registry(creditor_name) if creditor_name else None

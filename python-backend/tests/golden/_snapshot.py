...
EXCLUDED_FILES = {
    ...
    # Внутренне противоречивый документ: «Сумма требований» включает госпошлину
    # 23 719, но разбивка «в том числе» её не перечисляет (итог ≠ сумме строк).
    "andro/без маркеров заявл.docx",
}


# Корпус сужен до одной папки: работаем только со свежей конвертацией.
# Отсюда следует, что вне корпуса оказываются:
#  - «Заявления на проверку/» — те же документы, что в «НОвая конвертация/ФНС»;
#  - «Самобанкрот/» (верхний уровень) — конвертация от 08.07, вытесненная папкой
#    «НОвая конвертация/Самобанкрот» (те же 9 имён, версии от 13-15.07);
#  - «Правильные заявления/», «реализация/» — и так исключались ниже.
CORPUS_ROOT = "НОвая конвертация/"


def _is_excluded(rel: str) -> bool:
    """Файл вне корпуса, если он вне CORPUS_ROOT, в EXCLUDED_FILES, это ЛЮБОЙ PDF,
    заявление о взыскании или синтетическая болванка «БМ …». Работаем только с docx
    (PDF-распознавание менее точное), без взысканий и болванок.
    """
    # Всё вне свежей конвертации — не корпус (правила ниже действуют внутри неё).
    if not rel.startswith(CORPUS_ROOT):
        return True
    ...
    if rel.startswith("реализация/"):
        return True
    # Папка «Правильные заявления/» — дубли документов из других папок (по просьбе
    # Андрея убрана из тестов, чтобы не двоить корпус).
    if rel.startswith("Правильные заявления/"):
        return True
    return False

...
#
# Фаза 1 (контракт поля) сняла отсюда две пары: РФЛ1×double_spaces и
# Медведев×label_case ломались ТОЛЬКО через loanDebt, куда мутация переносила
# кусок другого текста. Как только поле перестало принимать не-суммы, мутация
# перестала на него влиять — устойчивость выросла как побочный эффект.
KNOWN_BRITTLE = {
    ...
    ("НОвая конвертация/Самобанкрот/Заявление на банкротство Медведев М.Ю.docx", "double_spaces"),  # documentType, sourceDocumentType
